...
BSTNode<E>* BST<E>::deletehelp(BSTNode<E>* rt, const E& e)
{
    ...
    {
        BSTNode<E>* temp = rt;
        if (rt->left() == NULL)//half node or leaf
        {
            rt = rt->right();
            delete temp;
        }
        else if (rt->right() == NULL)//half node or leaf
        {
            rt = rt->left();
            delete temp;
        }
        else//2 children, 2 subtree, a child a subtree
        {
...
template<typename E>
int BST<E>::getHeighthelp(BSTNode<E>* root)
{
    if(root==NULL)
        return 0;
    return 1+max(getHeighthelp(root->left()),getHeighthelp(root->right()));
}
int main()
{
    BST<int> tree;
